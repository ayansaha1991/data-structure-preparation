...
public class TaskScheduler {
    public static void main(String[] args) throws Exception {
        ...
        int t = 1;
        int totalTimeToComplete = findTotalTimeToComplete(tasks, t);
        System.out.println(totalTimeToComplete);
    }

    private static int findTotalTimeToComplete(char[] tasks, int idleTime) throws Exception {
        Map<Integer, Integer> taskCountMap = new HashMap<>();
        // create a map of Task as key and count of task as value
        for (int i = 0; i < tasks.length; i++) {
            taskCountMap.computeIfPresent(((int) tasks[i])-65, (key, oldValue) -> oldValue + 1);
            taskCountMap.putIfAbsent(((int) tasks[i])-65, 1);
        }
        // create a Max heap with task count values
        MinHeap minHeap = new MinHeap();
        PriorityQueue<Integer> priorityQueue = new PriorityQueue<>();
        for (int taskCount : taskCountMap.values()) {
            minHeap.insert(-taskCount);
            priorityQueue.add(-taskCount);
        }

        // queue for future tasks
        Queue<TasKAndTime> queue = new ArrayDeque<>();
        int currentTime = 0;
        // take the most frequent task from max-heap
        while (!priorityQueue.isEmpty()  || !queue.isEmpty()) {
            currentTime = currentTime +1;

            if (!priorityQueue.isEmpty()) {
                Integer taskToBeProcessed = priorityQueue.poll();
                Integer taskToBeQueued = taskToBeProcessed + 1;
                if (taskToBeQueued != 0) {
                    queue.add(new TasKAndTime(taskToBeQueued, currentTime+idleTime));
                }
            }

            if (!queue.isEmpty() && queue.peek().whenToBeProcessedAgain() == currentTime) {
                TasKAndTime task = queue.remove();
                priorityQueue.add(task.taskId());
            }
        }
...
record TasKAndTime(int taskId, int whenToBeProcessedAgain){}